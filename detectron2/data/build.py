...
def get_detection_da_dataset_dicts(names_s, names_t, filter_empty=True, min_keypoints=0, proposal_files=None):
    dataset_s = get_detection_dataset_dicts(
            names_s,
            filter_empty,
            min_keypoints,
            proposal_files,
        )
    dataset_t = get_detection_dataset_dicts(
            names_t,
            filter_empty,
            min_keypoints,
            proposal_files,
...
def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None):
    ...
    # Standard single-domain Detectron2 loader.
    if len(cfg.DATASETS.TRAIN) and not (len(cfg.DATASETS.TRAIN_S) and len(cfg.DATASETS.TRAIN_T)):
        if dataset is None:
    ...
    # Domain-adaptation source/target loader.
    if dataset is None:
        dataset_s, dataset_t = get_detection_da_dataset_dicts(
            cfg.DATASETS.TRAIN_S,
            cfg.DATASETS.TRAIN_T,
            filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
            min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
            if cfg.MODEL.KEYPOINT_ON
            else 0,
        ...
        _log_api_usage("dataset." + cfg.DATASETS.TRAIN_S[0])
        _log_api_usage("dataset." + cfg.DATASETS.TRAIN_T[0])
    ...
    if sampler is None:
        ...
        if sampler_name == "TrainingSampler":
            #sampler = TrainingSampler(len(dataset))
            sampler = TrainingSampler(min(len(dataset_s), len(dataset_t)))
        elif sampler_name == "RepeatFactorTrainingSampler":
            repeat_factors = RepeatFactorTrainingSampler.repeat_factors_from_category_frequency(
                dataset, cfg.DATALOADER.REPEAT_THRESHOLD
            )
...
@configurable(from_config=_train_loader_from_config)
def build_detection_da_train_loader(
    dataset_s, dataset_t, *, mapper, sampler=None, total_batch_size, aspect_ratio_grouping=True, num_workers=0
):
    """
    Build a dataloader for object detection with some default features.
    This interface is experimental.
    ...
    Returns:
        torch.utils.data.DataLoader:
            ...
            by the ``mapper``.
    """
    if isinstance(dataset_s, list):
        dataset_s = DatasetFromList(dataset_s, copy=False)
        dataset_t = DatasetFromList(dataset_t, copy=False)
    if mapper is not None:
        dataset_s = MapDataset(dataset_s, mapper)
        dataset_t = MapDataset(dataset_t, mapper)
    if sampler is None:
        sampler = TrainingSampler(min(len(dataset_s), len(dataset_t)))
    assert isinstance(sampler, torch.utils.data.sampler.Sampler)
    return build_batch_data_loader(
